defmodule ApiWeb.Api.V1_1.UserView do
  @moduledoc false
  import Ecto.Query, warn: false
  use ApiWeb, :view
  alias ApiWeb.Api.V1_1.{UserView}
  ...
  def render("user.json", %{user: user, message: user_creation_message}) do
    ...
    %{
      ...
      jetzy_select_funnel: (Map.get(user, :jetzy_select_status) || :denied) != :denied && :standard || :disabled, # longer term temp.
      jetzy_select_subscription:  (Map.get(user, :jetzy_select_status) || :denied),
      view_by: 1,
  ...
  def render("user.json", %{user: user}) do
    %{
      ...
      chat_settings: Map.get(user, :chat_settings) || nil,
      jetzy_exclusive_status: Map.get(user, :jetzy_exclusive_status) || :denied,
      jetzy_select_status: false, # Hard code to work around issue in build.
      jetzy_select_funnel: (Map.get(user, :jetzy_select_status) || :denied) != :denied && :standard || :disabled, # longer term temp.
      jetzy_select_subscription:  (Map.get(user, :jetzy_select_status) || :denied),
