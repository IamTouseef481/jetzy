...
config :api, :cdn,
       path: "/mnt/images/jetzy-stage",
       upload: "/mnt/upload"

config :api, :stripe,
       webhook_secret: get_env.("JETZY_STAGE_STRIPE_WEBHOOK_SECRET", __ENV__, :required)
       
config :api, :schema,
       mssql_username: get_env.("JETZY_STAGE_MSSQL_USER", __ENV__, :required),
       mssql_password: get_env.("JETZY_STAGE_MSSQL_PASSWORD", __ENV__, :required),
       mssql_database: get_env.("JETZY_STAGE_MSSQL_DATABASE", __ENV__, :required),
       mssql_hostname: get_env.("JETZY_STAGE_MSSQL_HOST", __ENV__, :required),
       pg_db_user: get_env.("JETZY_STAGE_DB_USER_NAME", __ENV__, :required),
       pg_db_password: get_env.("JETZY_STAGE_DB_PASSWORD", __ENV__, :required),
       pg_db_name: get_env.("JETZY_STAGE_DB_NAME", __ENV__, :required),
       pg_db_host: get_env.("JETZY_STAGE_DB_HOST_NAME", __ENV__, :required),
       pg_db_pool: get_env.("JETZY_STAGE_DB_POOL_SIZE", __ENV__, :required),
       pg_db_odbc: get_env.("JETZY_STAGE_DB_URL", __ENV__, :required),
       pg_db_port: get_env.("JETZY_STAGE_DB_PORT", __ENV__, :required)


#-------------------------------------------------------------------------------
# fcmex (SMS) config
#-------------------------------------------------------------------------------
fcm_server_key = get_env.("JETZY_STAGE_FCM_SERVER_KEY", __ENV__, :required)
fcm_sender_id = get_env.("JETZY_STAGE_FCM_SENDER_ID", __ENV__, :required)
...
#-------------------------------------------------------------------------------
google_app_id = get_env.("JETZY_STAGE_GOOGLE_APP_ID", __ENV__, :optional)
google_app_secret = get_env.("JETZY_STAGE_GOOGLE_APP_SECRET", __ENV__, :optional)
...
#-------------------------------------------------------------------------------
guardian_secret = get_env.("JETZY_STAGE_GUARDIAN_SECRET", __ENV__, :required)
...
#-------------------------------------------------------------------------------
config :ex_aws,
       access_key_id: get_env.("JETZY_STAGE_AWS_ACCESS_KEY_ID", __ENV__, :required),
       secret_access_key: get_env.("JETZY_STAGE_AWS_SECRET_ACCESS_KEY", __ENV__, :required),
       region: get_env.("JETZY_STAGE_AWS_REGION", __ENV__, :required)

#-------------------------------------------------------------------------------
#SendGrid config
#-------------------------------------------------------------------------------
sendgrid_api_key = get_env.("JETZY_STAGE_SENDGRID_API_KEY", __ENV__, :required)
config :sendgrid,
...
host = get_env.("JETZY_STAGE_API_HOST_URL", __ENV__, :required)
secret_key_base = get_env.("JETZY_STAGE_API_SECRET_KEY_BASE", __ENV__, :required)
live_view_signing_salt = get_env.("JETZY_STAGE_LIVE_VIEW_SIGNING_SALT", __ENV__, :required)
port = get_env.("JETZY_STAGE_API_PORT", __ENV__, :required)
new_relic_name = get_env.("JETZY_STAGE_NEW_RELIC_NAME", __ENV__, :required)
new_relic_license = get_env.("JETZY_STAGE_NEW_RELIC_LICENSE", __ENV__, :required)
...
config :api, :configuration,
       referral_code_url: get_env.("JETZY_STAGE_REFERRAL_CODE_URL", __ENV__, :optional)
       
config :api, :firebase,
       api_key: get_env.("JETZY_STAGE_FIREBASE_WEB_API_KEY", __ENV__, :required)
...
import_config "../cron/stage_cron.exs"