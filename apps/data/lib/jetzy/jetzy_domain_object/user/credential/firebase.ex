...
#-------------------------------------------------------------------------------
defmodule Jetzy.User.Credential.Firebase do
  ...
  Noizu.SimpleObject.noizu_struct() do
    @required true
    @pii :level_0
    restricted_field :firebase_user
  end
  ...
  def __as_record__(%{__struct__: PersistenceLayer, schema: JetzySchema.PG.Repo} = _layer, identifier, settings, _context, options) do
    ...
    %JetzySchema.PG.User.Credential.Firebase.Table{
      identifier: identifier,
      firebase_user: settings.firebase_user,
  ...
  def query_key(%__MODULE__{} = this), do: {:firebase, this.firebase_user}
  ...
  def by_setting__ecto(setting, _context, _options) do
    query = from c in JetzySchema.PG.User.Credential.Firebase.Table,
                 where: c.firebase_user == ^setting.firebase_user,
                 select: c,
                 limit: 1
