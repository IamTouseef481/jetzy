...
defmodule Jetzy.Entity.Contact.Channel.Repo.TypeHandler do
  ...
  # from_partial
  #----------------------
  def from_partial(%{__struct__: Jetzy.Entity.Contact.Channel.Repo} = v, context, options) do
    entities = Enum.map(v.entities, &(Jetzy.Entity.Contact.Channel.TypeHandler.from_partial(&1, context, options)))
               |> Enum.filter(&(&1))
    %Jetzy.Entity.Contact.Channel.Repo{v| entities: entities, length: length(entities)}
  end
  def from_partial(v, context, options) when is_list(v) do
    entities = Enum.map(v, &(Jetzy.Entity.Contact.Channel.TypeHandler.from_partial(&1, context, options)))
               |> Enum.filter(&(&1))
    %Jetzy.Entity.Contact.Channel.Repo{entities: entities, length: length(entities)}
  end
  def from_partial(_, _context, _options) do
    %Jetzy.Entity.Contact.Channel.Repo{entities: [], length: 0}
  end
    ...
    update_in(
      ...
      fn (x) ->
        ...
        entities = Enum.map(
          interests.entities || [],
          fn
            (%{identifier: nil} = s) -> Jetzy.Entity.Contact.Channel.Repo.create(s, context, options)
            (v) -> v
          end
        )
        %Jetzy.Entity.Contact.Channel.Repo{interests | entities: entities}
      end
    )
  end
  ...
  def post_create_callback(field, entity, context, options) do
    ...
    case interests do
      %Jetzy.Entity.Contact.Channel.Repo{entities: entities} ->
        entities
        |> Enum.filter(&(is_struct(&1) && &1.__transient__[:persist?]))
        |> Enum.map(&( Jetzy.Entity.Contact.Channel.Repo.create(&1, context, options)))
      _ -> nil
    end
    entity
  end
