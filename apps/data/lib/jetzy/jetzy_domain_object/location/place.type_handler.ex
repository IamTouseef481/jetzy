...
defmodule Jetzy.Location.Place.TypeHandler do
  ...
  # from_partial
  #--------------------------------------
  def from_partial(%{__struct__: Jetzy.Location.Place.Entity} = v, _context, _options), do: v
  ...
  def pre_create_callback(field, entity, context, options) do
    update_in(
      ...
      [Access.key(field)],
      fn (x) ->
        from_partial(x, context, options)
      end
    )
  end
  def pre_create_callback!(field, entity, context, options) do
