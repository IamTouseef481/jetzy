defmodule Jetzy.Vocation.TypeHandler do
  ...
  # from_partial
  #--------------------------------------
  def from_partial(%{__struct__: Jetzy.Vocation.Entity} = v, _context, _options), do: v
  def from_partial(%{name: name}, _context, options) do
    now = options[:current_time] || DateTime.utc_now()
    %Jetzy.Vocation.Entity{
      name: name,
    ...
    update_in(
      ...
      fn (x) ->
        ...
        if pi && pi.identifier == nil do
          Jetzy.Vocation.Repo.create(pi, context, options)
        else
          pi
        end |> Noizu.ERP.ref()
      end
