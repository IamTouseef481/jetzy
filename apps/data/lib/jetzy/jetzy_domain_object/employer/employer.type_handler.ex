defmodule Jetzy.Employer.TypeHandler do
  ...
  # from_partial
  #--------------------------------------
  def from_partial(%{__struct__: Jetzy.Employer.Entity} = v, _context, options), do: v
  def from_partial(%{name: name}, context, options) do
    now = options[:current_time] || DateTime.utc_now()
    %Jetzy.Employer.Entity{
      name: name,
    ...
    update_in(
      ...
      fn (x) ->
        ...
        if pi && pi.identifier == nil do
          Jetzy.Employer.Repo.create(pi, context, options)
        else
          pi
        end |> Noizu.ERP.ref()
      end
