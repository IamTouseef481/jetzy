...
defmodule Jetzy.CMS.Article.Post.TypeHandler do
  ...
  def pre_create_callback(field, entity, context, options) do
    update_in(
      ...
      fn (v) ->
        case v do
          v = %{__struct__: Jetzy.CMS.Article.Post.Entity} ->
            cond do
              v.identifier -> v
  ...
  def pre_update_callback(field, entity, context, _options) do
    ...
    update_in(
      ...
      fn (v) ->
        case v do
          %{__struct__: Jetzy.CMS.Article.Post.Entity, identifier: identifier} when identifier != nil ->
  ...
  def from_json(format, field, json, context, options) do
    {s, j} = case json[Atom.to_string(field)] do
               vs_json = %{"identifier" => identifier} ->
                 if existing = Jetzy.CMS.Article.Post.Entity.entity!(identifier, context) do
                   {existing, vs_json}
                 else
                   editor = cond do
  ...
  def __sphinx_expand_field__(field, indexing, _settings) do
    ...
    [
      {:"#{field}_title", __MODULE__, put_in(indexing, [:sub], :title)},
  ...
  def __sphinx_encoded__(_field, entity, indexing, _settings) do
    ...
    cond do
      ...
      indexing[:sub] == :title -> value.title
